...
every input. Phase differences alone do not imply orthogonality or a maximally
mixed average state; those properties depend on the input distribution.
"""
...
def encoded_density_matrix(feature_map, x: np.ndarray, n_qubits: int,
    ...
    for i in range(0, len(x), batch_size):
        ...
        if psi.ndim == 1:
            psi = psi[None, :]
        rho += psi.T @ psi.conj()          # sum_b |psi_b><psi_b|
        total += len(psi)
    return rho / total
...
def compressibility(feature_map, x: np.ndarray, n_qubits: int,
    ...
    return {
        ...
        # Descriptive heuristic only; this threshold is not a theorem and
        # provides no bound on anomaly detection performance.
